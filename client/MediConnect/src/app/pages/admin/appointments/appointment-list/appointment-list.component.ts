...
export class AppointmentListComponent implements OnInit {
  ...
  public colDefs: ColDef[] = [
    ...
    },
    {
      ...
      minWidth: 150,
      valueGetter: (params) => this.getPatientNameById(params.data.patientId) ?? 'N/A',
    },
    {
      field: 'doctorName',
      headerName: 'Doctor Name',
      minWidth: 150,
      valueGetter: (params) => this.getDoctorNameById(params.data.doctorId) ?? 'N/A',
    },
    {
      ...
      this.patientService.getPatients().subscribe({
        next: (patients) => {
          this.patientNameMap = new Map(
            patients.content.map((patient: any) => [patient.id, patient.user.fullName])
          );
