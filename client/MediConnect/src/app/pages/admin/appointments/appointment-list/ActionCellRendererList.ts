...
@Component({
  ...
  template: `
    <!-- Dialog Content for Viewing Schedule -->
    <hlm-dialog>
      <button
      ...
      <hlm-dialog-content
        class="max-w-[500px] bg-white lg:max-w-[700px] text-left h-[82vh] overflow-y-auto"
        *brnDialogContent="let ctx"
      >
        <hlm-dialog-header class="w-full text-left">
          <h3 hlmDialogTitle>Appointment Information</h3>
          <p hlmDialogDescription>
            Detailed information regarding the appointmnent's data
          </p>
        </hlm-dialog-header>
    ...
    <a
      [routerLink]="['/admin/dashboard/appointments', params.data.appointmentId]"
    >
      <button
...
export class ActionCellRendererList implements ICellRendererAngularComp {
  ...
  onDelete(): void {
    const parentComponent = this.params.context as AppointmentListComponent;
    const appoinmentId = this.params.data.appointmentId;
    console.log(appoinmentId);
    if (parentComponent && parentComponent.deleteOneAppointment) {
      parentComponent.deleteOneAppointment(appoinmentId);
    } else {
