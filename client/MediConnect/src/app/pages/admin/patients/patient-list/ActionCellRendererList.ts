    ...
    <hlm-alert-dialog>
      ...
      >
        ...
        <hlm-alert-dialog-footer>
          ...
          <button hlmAlertDialogAction class="bg-gray-800 text-white" (click)="onDelete()">Delete patient</button>
        </hlm-alert-dialog-footer>
      </hlm-alert-dialog-content>
    </hlm-alert-dialog>
  `,
...
export class ActionCellRendererList implements ICellRendererAngularComp {
  ...
  onDelete(): void {
    const parentComponent = this.params.context as PatientListComponent;
    if (parentComponent && parentComponent.deleteOnePatient) {
      parentComponent.deleteOnePatient(this.params.data.id); // Call deleteUser on parent component
    } else {
